...
In `drake` version 7.5.0 and above, `make(recover = TRUE)` can salvage old targets from the distant past. This may not be a good idea if your external dependencies have changed a lot over time (R version, package environment, etc) but it can be super handy.
...
You can even rename your targets! Just be aware that this invalidates downstream targets.
