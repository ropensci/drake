...
- Add experimental support for URLs in `file_in()` and `file_out()`. If `drake` detects a file name beginning with "http://", "https://", or "ftp://", it tries to get an ETag to track the remote data. If it cannot find one, it tries to get a Last-Modified time stamp instead. If it cannot find either, it warns the user and errs on the side of not invalidating the target.
