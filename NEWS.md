...
- Use `formatR::tidy_source()` instead of `parse()` in `tidy_command()` (in `R/dependencies.R`). Previously, `drake` was having problems with an edge case: as a command, the literal string `"A"` was interpreted as the symbol `A` after tidying. With `tidy_source()`, literal quoted strings should stay literal quoted strings in commands. This may put some targets out of date in old projects, but that is unlikely. The change should be mostly back-compatible.
