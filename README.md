...
[![](http://cranlogs.r-pkg.org/badges/drake)](http://cran.rstudio.com/package=drake)
...
However, if you are simply debugging and testing a project, you can relax this reproducible reactivity using [triggers](https://github.com/wlandau-lilly/drake/blob/master/vignettes/debug.Rmd#test-with-triggers). For example, if you do not care about reproducibility and just want `drake` to be an ordinary job scheduler, use the `"missing"` trigger. That way, targets will still be built in the correct sequence of interdependent stages, but any targets that already exist will be skipped. You lose reproducibility, but execution can be much faster.

```r
make(..., trigger = "missing")
```
