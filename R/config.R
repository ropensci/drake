...
#'
#' @param ordinances `config$ordinances`, where `config` is the return value
#'   from a prior call to `drake_config()`. If your plan or environment
#'   have changed since the last `make()`, do not supply a `ordinances` argument.
...
drake_config <- function(
  ...
  if (is.null(ordinances)) {
    ordinances <- create_drake_ordinances(
      plan = plan,
      targets = targets,
      envir = envir,
      verbose = verbose,
