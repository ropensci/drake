conserve_memory = function(next_targets, config){
  downstream = lapply(next_targets, function(vertex)
    subcomponent(config$graph, v = vertex, mode = "out")$name) %>%
    unlist %>% unique
  already_loaded = ls(envir = config$envir) %>%
    intersect(y = config$plan$target)
  load_these = 
    nonfile_target_dependencies(targets = next_targets, config = config) %>%
