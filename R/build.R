build = function(target, args){
  ...
  current = is_current(target = target, dependency_hash = dependency_hash,
  ...
  args$cache$set(key = target, value = dependency_hash, 
    namespace = "depends")
...
build_target = function(target, args){
  ...
  if(is_file(target)) 
    store_file(target, file_hash = NULL, imported = FALSE, args = args)
...
import_target = function(target, file_hash, args){
  ...
  if(is_file(target)){
    store_file(target = target, file_hash = file_hash, 
...
store_file = function(target, file_hash, imported, args){
  filename = unquote(target)
  if(!length(file_hash)) file_hash = md5sum(filename)
  args$cache$set(key = target, 
    value = list(type = "file", value = file_hash, imported = imported))
  args$cache$set(key = target, value = file.mtime(filename), 
    namespace = "filemtime")
}

store_function = function(target, value, imported, args){
