...
append_build_times = function(nodes, cache){
  ...
  nodes[timed, "label"] = paste(nodes[timed, "label"], time_labels, sep = "\n")
  nodes
}
...
arrange_nodes = function(nodes, parallelism, graph, targets, imports){
  if(parallelism == "Makefile") # Imports and targets are parallelized separately.
    resolve_levels_Makefile(nodes = nodes, graph = graph, 
...
categorize_nodes = function(nodes, 
                            functions, imports, in_progress,
                            missing, outdated, targets){
...
configure_nodes = function(nodes, plan, envir, parallelism, graph, cache, 
                           functions, imports, in_progress,
                           missing, outdated, targets, font_size,
                           build_times){
  force(envir)
  ...
  nodes = categorize_nodes(nodes = nodes, functions = functions,
                           imports = imports, in_progress = in_progress,
                           missing = missing, outdated = outdated, targets = targets)
  nodes = arrange_nodes(nodes = nodes, parallelism = parallelism, graph = graph, 
                        imports = imports, targets = targets)
...
hover_text = function(nodes, plan, targets){
  ...
  nodes[targets, "hover_label"] = plan[plan$target %in% targets, "command"] %>%
