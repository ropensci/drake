...
test_that("evaluate, expand, and gather", {
  ...
  y = data.frame(
    ...
    command = c("simulate(center = 1, scale = SIGMA)",
  ...
  y = data.frame(
    target = c("data_rep1_1", "data_rep1_2", 
               "data_rep2_1", "data_rep2_2"),
    command = c("simulate(center = 1, scale = a)",
  ...
  expect_equal(12, nrow(x5), length(unique(x5$target)), 
    length(unique(x5$command)))
  
  x6 = gather(x)
  y = data.frame(target = "target", 
...
test_that("analyses and summaries", {
  ...
  x = data.frame(
    target = c("regression1_small", "regression1_large", 
               "regression2_small", "regression2_large"),
    command = c("reg1(small)", "reg1(large)", 
                "reg2(small)", "reg2(large)"),
    stringsAsFactors = F)
  ...
  results = summaries(summary_types, analyses, datasets,
                      gather = NULL)
  x = data.frame(
    target = c("summ_regression1_small", "summ_regression1_large", "summ_regression2_small", 
               "summ_regression2_large", "coef_regression1_small", "coef_regression1_large", 
               "coef_regression2_small", "coef_regression2_large"),
    command = c("summary(regression1_small)", "summary(regression1_large)", 
                "summary(regression2_small)", "summary(regression2_large)", "coef(regression1_small)", 
                "coef(regression1_large)", "coef(regression2_small)", "coef(regression2_large)"),
    stringsAsFactors = FALSE)
  expect_equal(results, x)

  summary_types = plan(summ = summary(..analysis.., ..dataset..),
                       coef = coef(..analysis..))
  results = summaries(summary_types, analyses, datasets,
                      gather = c("list", "rbind"))
  x = data.frame(
    target = c("summ_regression1_small", "summ_regression1_large", "summ_regression2_small", 
               "summ_regression2_large", "coef_regression1_small", "coef_regression1_large", 
               "coef_regression2_small", "coef_regression2_large"),
    command = c("summary(regression1_small, small)", "summary(regression1_large, large)", 
                "summary(regression2_small, small)", "summary(regression2_large, large)", 
                "coef(regression1_small)", "coef(regression1_large)", "coef(regression2_small)", 
