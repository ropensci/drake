...
For more control over how R is called on each target, use the `recipe_command` argument. By default, the `recipe_command` is `"Rscript -e"`.
...
That way, a target named `your_target` is built in the `Makefile` with the following. (To verify, read the `Makefile` output by `make()`.)

<pre><code>Rscript -e 'drake::mk("your_target", "$(DRAKE_CACHE")'
</code></pre>

But you can change the recipe to something else. For example, to save some time and skip the loading of the `methods` package, you might use `R -q -e`.
...
<pre><code>R -q -e 'drake::mk("your_target", "$(DRAKE_CACHE")'
</code></pre>
