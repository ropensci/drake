...
# Choose a GitHub release.
devtools::install_github(
  "wlandau-lilly/drake@v4.4.0",
  build = TRUE
)
...
library(drake)
load_basic_example()     # Also (over)writes report.Rmd.
vis_drake_graph(my_plan) # Hover, click, drag, zoom, pan.
outdated(my_plan)        # Which targets need to be (re)built?
missed(my_plan)          # Are you missing anything from your workspace?
check_plan(my_plan)      # Is your workflow plan okay? Missing files?
make(my_plan)            # Run the workflow.
diagnose(large)          # Error info if the target "large" failed to build.
...
vignette("quickstart") # https://cran.r-project.org/package=drake/vignettes
```
...
outdated()
missed()
vis_drake_graph() # Now with subgraphs too.
dataframes_graph()
render_drake_graph()
read_drake_graph()
deps()
