...
The internal functions `drake:::meta()` and `drake:::meta_list()` compute the metadata on each target that `drake` uses to decide which targets to build and which to skip (via `drake:::should_build_target()`). To help with debugging and testing, `drake` stores the metadata computed right before each target's possible build. You can view it with `read_drake_meta()` if you previously ran `make(..., store_meta = TRUE)`.

```{r readdrakemeta}
str(head(read_drake_meta()))
```
...
debug_plan <- plan_drake(x = 1, y = f(x))
debug_plan
...
1. the most recent error log of each target.
...
The `load_basic_example()` function loads the [basic example](https://github.com/wlandau-lilly/drake/tree/master/inst/examples/basic) right into your workspace. The workflow plan data frame, workspace, and import files are set up for you. Only `make(my_plan)` is left to you.

`Drake` has [many more built-in examples](https://github.com/wlandau-lilly/drake/tree/master/inst/examples). To see your choices, use
