...
<iframe
src = "https://cdn.rawgit.com/wlandau-lilly/drake/21f83e6a/images/good-commands.html"
...
<iframe
src = "https://cdn.rawgit.com/wlandau-lilly/drake/21f83e6a/images/bad-commands.html"
width = "100%" height = "600px" allowtransparency="true"
style="border: none; box-shadow: none">
</iframe>
...
2. `Drake` does not know about the `data.csv` file at all since it is not explicitly mentioned in a workflow plan command. So when `data.csv` changes, `make(bad_plan)` will not rebuild `my_data`.
3. In `make(bad_plan, jobs = 2)`, `drake` will try to build `my_data` and `my_analysis` at the same time even though `my_data` must be built first.
