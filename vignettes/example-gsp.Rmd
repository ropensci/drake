...
output_plan <- drake_plan(
  ggsave(
    filename = file_output("rmspe.pdf"),
...
Because of the mention of `knitr_input()` above, `make()` will look dependencies inside `report.Rmd` (targets mentioned with `loadd()` or `readd()` in active code chunks). That way, whenever a dependency changes, `drake` will rebuild `report.md`. For that to happen, we need `report.Rmd` to exist before the call to `make()`. For this example, you can find [report.Rmd here](https://github.com/ropensci/drake/blob/master/inst/examples/gsp/report.Rmd) for this example.
