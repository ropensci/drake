...
If you wish to tinker with the example yourself, you can generate the [underlying code files](https://github.com/ropensci/drake/tree/master/inst/examples/gsp) with `drake_example("gsp")` and then step through `run.R`.
...
The new `gsp` folder now includes a file structure of a serious `drake` project, plus an `interactive-tutorial.R` to narrate the example.
