...
In theory, it should be possible to leverage serious databases using `storr_dbi()`. However, this feature is currently experimental, and it crashes when parallel processing is activated. And if the underlying database groups multiple targets together in the same file, simultaneous writes will almost surely cause race conditions, so `storr_dbi()` might not be an appropriate `drake` cache anyway. In addition, single-quoted keys do not currently play nicely with `storr_dbi()` caches, as [documented in the storr issue tracker](https://github.com/richfitz/storr/issues/60). That means you cannot use file targets.
...
# https://github.com/richfitz/storr/issues/60
my_plan <- my_plan[my_plan$target != "'report.md'", ]
