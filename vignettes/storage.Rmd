...
1. Be careful about parallel computing. For example the `storr::storr_dbi()` cache is not threadsafe. Either use no parallel computing at all or set `parallelism = "future"` with `caching = "master"`. The `"future"` backend is currently experimental, but it allows the master process to do all the caching in order to avoid race conditions.
