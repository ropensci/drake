...
`Drake` supports [storr_rds()](https://github.com/richfitz/storr) caches, but currently struggles with non-`rds` caches such as  `storr_dbi()`. And some in-memory caches will likely never be supported for `make(..., parallelism = "Makefile")`.
