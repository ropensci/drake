...
Drake uses [codetools::findGlobals()](https://cran.r-project.org/package=codetools) in the backend to look for dependencies, which can be fooled. For example, suppose you have a custom function `f` in your workspace.
...
When drake looks for the dependencies of `f`, it will fail to recognize the objects `a` and `x`, the function `digest()`, and the file `'my_file.txt'`. Objects `a` and `x` are referenced with quoted strings, not symbols, which tricks drake. The function `digest()` is referenced with the scoping rule `::`, so [codetools::findGlobals()](https://cran.r-project.org/package=codetools) does not detect it. Lastly, because `'my_file.txt'` is inside a function and not a command in your workflow plan data frame, drake will not reproducibly track it.
