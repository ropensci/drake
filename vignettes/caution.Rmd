...
In addition, if you import a function like `f <- Vectorize(g, ...)` and hover over the `f` node in `plot_graph(hover = TRUE)`, then you will see the body of `g()`, not the body of `f()`.
...
# Advanced caching and hashing

In theory, you could try any [storr](https://github.com/richfitz/storr)-like cache for drake.

```r
my_cache <- storr::storr_rds("my_cache", mangle_key = TRUE)
make(..., cache = my_cache
```

Code like the above is expected to work. However, only file system caches are currently supported, and in-memory caches will likely never be supported for `Makefile` parallelism. That means you should be wary of `storr::storr_dbi()`, especially at the moment.
