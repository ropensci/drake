...
## Name conflict between `future::plan()` and `drake::plan()`

This is unlikely to be fixed in the future. So to avoid name conflicts, you can use the scoping operator `::` to specify the package of origin, or you can use `backend()` instead of `future::plan()` and `workflow()` instead of `drake::plan()`.
