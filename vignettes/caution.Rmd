...
You must properly install `drake` using `install.packages()`, `devtools::install_github()`, or similar. It is not enough to use `devtools::load_all()`. Yes, `load_all()` will load `drake` into your personal R session, but the parallel computing functionality deploys separate R sessions on its own, and those sessions need to use `require()` to load `drake` separately.
