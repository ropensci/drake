...
# Projects built with drake <= 4.4.0 are not back compatile with drake >= 5.0.0

Version 5.0.0 of `drake` comes with changes to the internals of caching that increase speed and open up possibilities for future development. However, if you have a project built with version 4.0.0, `drake` >= 5.0.0 will think all your targets are out of date. Hence, a simple `make()` will quit in error to stop you from doing any damage. To move your project to 5.0.0, you have two options.

1. Run your project from scractch with `make(..., force = TRUE)`
1. Use `migrate()` to convert your project to the new format. The `migrate()` function
...
## Advanced: find out why a target is out of date.
