...
It is usually that simple. Every workflow has a network of dependencies that dictates what `drake` can run in parallel and when.
...
| --------------------- |:-------------:| -----:|
| Algorithm: persistent | "mclapply", "parLapply" | "future_lapply" |
| Algorithm: transient  | | "future", "Makefile" |
| Algorithm: staged     | "mclapply_staged", "parLapply_staged" | |
...
# Persistent and transient scheduling
...
In each of these calls to `make()`, three processes launch: two workers and one master. Whenever a worker is idle, the master assigns it the next available target (whose dependencies have been built). The workers keep running until there are no more targets to build. The `predict_runtime()` and `predict_load_balancing()` functions emulate persistent workers, and the predictions also apply to transient workers. See the [timing guide](https://ropensci.github.io/drake/articles/timing.html) for a demonstration.

Persistent workers are great because they minimize overhead: all the workers are created at the beginning, and then you never have to create any more for the rest of the runthrough. Unfortunately, computing clusters usually limit the amount of time each worker can stay running. That is why `drake` also supports transient workers in backends `"future"` and `"Makefile"`. Here, the master process creates a new worker for each target individually, and the worker dies after it finishes its single target. For the `"future"` backend, the master is just the existing process calling `make()`.
...
Here, the dependency network is divided into separate stages of conditionally independent targets. Within each stage, `drake` uses `mclapply()` or `parLapply()` to process the targets in parallel. Generally, those stages are the columns int the dependency graph. But if some targets are already up to date, `drake` skips ahead to pack as many targets as possible into the next stage.

<img src="./staged.png" alt="staged" align="center" style = "border: none; float: center;" width = "500px">

Stages run one after the other, so the slowest target in the current stage needs to complete before the next stage begins. So we lose a lot of parallel efficiency. However, because there is no formal master process in each stage, overhead is extremely low. This lack of overhead can make staged parallelism a great choice for projects with a small number of large stages: tall dependency graphs with most of the work in the tallest stages.
