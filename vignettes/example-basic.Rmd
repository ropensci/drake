...
1. For [dynamic `knitr` reports](https://yihui.name/knitr/), targets and imports mentioned in calls to `readd()` and `loadd()` in active code chunks. `Drake` treats these items as dependencies of the compiled output (say, `'report.md'` or `report.html`). **The workflow plan command must call `knit()` or `render()` explicitly.** Examples of acceptable commands:
    - `knit('report.Rmd')`
