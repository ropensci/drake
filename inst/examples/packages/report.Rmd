...
If you wish to tinker with the example yourself, you can generate the [underlying code files](https://github.com/wlandau-lilly/drake/tree/master/inst/examples/application) with `drake_example("application")` and then step through `run.R`.

This `report.Rmd` file will not knit on its own until `run.R` has generated enough output.
